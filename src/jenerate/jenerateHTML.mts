...
async function processRelSnippet(
  ...
  return await processSnippetImpl(dom, target, "href", ref, dependencies, env);
}
...
async function processForEach(
  ...
  if (typeof src === "string") {
    ...
    if (typeof csvContent !== "undefined") {
      ...
      const selectLocation = getLocationString(getLocation(dom, selectAttr));
      ...
      for (const row of csvContent.rows) {
        ...
        const rowDom = new JSDOM(
          ejs.render(
            target.innerHTML.replaceAll(/&lt;%=\s*(.*?)\s*%&gt;/g, "<%= $1 %>"),
            rowEnv
          ),
          {
...
function compareDOMNodes(a: Node, b: Node): number {
  const following = a.compareDocumentPosition(b) & DOCUMENT_POSITION_FOLLOWING;
  if (following) {
    return -1; // 'a' comes before 'b'
  }

  const preceeding = a.compareDocumentPosition(b) & DOCUMENT_POSITION_PRECEDING;
  if (preceeding) {
  ...
  if (node) {
    switch (node.nodeType) {
      case ATTRIBUTE_NODE: {
        ...
        if (ownerElement) {
          const parentLocation: ElementNodeLocation | null | undefined =
            dom.nodeLocation(ownerElement);
...
export function getRelativeUrl(
  ...
  if (getAuthority(from) === getAuthority(to)) {
    return (
      relativePosixPath(posixDirname(from.pathname), to.pathname) + to.search
    );
  }
}
