        ...
        if (typeof csvContent !== "undefined") {
            ...
            if (!selectAttr) {
                throw new JenerateError(
                    [
                        `${ref}${getLocationString(location)} - No columns selected.`,
            ...
            if (unknownSelection.length > 0) {
                throw new JenerateError(
                    [
                        `${ref}${selectLocation} - Selected column not found.`,
            ...
            const unselectedParams = Object.entries(templateParameters)
                ...
                .map(
                    ([column, location]) =>
                        `\t${column} - ${ref}${getLocationString(location)}`,
                );

            if (unselectedParams.length > 0) {
                throw new JenerateError(
                    [
                        `${ref} - The following name(s) are used in a template without first being selected`,
                        ...unselectedParams,
                        `Fix the spelling of the name(s) in the template or add the problematic name(s) to the 'select' attribute at ${ref}${selectLocation}`,
                    ].join("\n"),
                );
            }
...
function throwJenerateError(
    ...
    const origin = `${ref}${getLocationString(location)}`;
    ...
    if (isNodeError(error)) {
        switch (error.code) {
            case "ENOENT": {
                ...
                throw new JenerateError(
                    `${origin} - Not found: ${relativePath(ref, error.path)}`,
                    {
                        cause: error,
                    },
                );
            ...
            case "EPERM": {
                ...
                throw new JenerateError(
                    `${origin} - Access denied: ${relativePath(ref, error.path)}`,
                    { cause: error },
                );
            }
    ...
    throw new JenerateError(`${ref}${location} - ${message}`, { cause: error });
}
